...
export default function Login() {
  ...
  const handleOAuthLogin = (provider) => {
    // NOTE: There is no /auth/google or /auth/github route on the backend
    // yet -- OAuth was never actually implemented server-side. Redirecting
    // there would take the user to a dead 404 page. Until real OAuth
    // exists, fail loudly but safely instead of navigating away.
    setError(
      `${provider === "google" ? "Google" : "GitHub"} sign-in isn't available yet -- please use your email and password.`
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // which requires application/x-www-form-urlencoded with fields named
      // "username"/"password" -- not JSON. Sending JSON here always failed
      // with a 422 first, then fell back to this format on every login,
      // doubling latency for no benefit. Just send the right format directly.
